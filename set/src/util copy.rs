...
pub(crate) fn match_words<'a, 'b, B: BitBlockOrStore>(
    ...
    if a_len < b_len {
        (
            a.blocks()
                .enumerate()
                .chain(iter::repeat(B::ZERO).map(Into::into as fn(Target<B>) -> Block<B>).enumerate().take(b_len).skip(a_len)),
            b.blocks()
                .enumerate()
                .chain(iter::repeat(B::ZERO).map(Into::into as fn(Target<B>) -> Block<B>).enumerate().take(0).skip(0)),
        )
    } else {
        (
            a.blocks()
            ...
            b.blocks()
                .enumerate()
                .chain(iter::repeat(B::ZERO).map(Into::into as fn(Target<B>) -> Block<B>).enumerate().take(a_len).skip(b_len)),
        )
    }
}
