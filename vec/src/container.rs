...
pub trait BitContainer: Clone {
    type Block: BitBlock;

    const BITS: usize = Self::Block::BITS;
    const BYTES: usize = Self::Block::BYTES;
    const ONE: Self::Block = Self::Block::ONE;
    const ZERO: Self::Block = Self::Block::ZERO;
...
impl<T: BitBlock> BitContainer for Vec<T> {
    type Block = T;
    type Alloc = ();
...
impl<A: smallvec::Array> BitContainer for smallvec::SmallVec<A>
where
    A::Item: BitBlock,
{
    type Block = A::Item;
