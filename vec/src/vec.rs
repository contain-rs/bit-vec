...
impl<C: BitContainer> ops::Index<usize> for BitVec<C::Block, C> {
    type Output = bool;
...
impl<C: BitContainer> BitVec<C::Block, C> {
    ...
    #[inline]
    pub fn new_general_in(alloc: C::Alloc) -> Self {
    ...
    pub fn from_elem_general(len: usize, bit: bool) -> Self {
        let nblocks = blocks_for_bits::<C::Block>(len);
        let mut storage: C = C::with_capacity(nblocks);
        storage.extend(iter::repeat_n(
            if bit { !C::ZERO } else { C::ZERO },
            nblocks,
        ));
    ...
    pub fn with_capacity_general(capacity: usize) -> Self {
        BitVec {
            storage: C::with_capacity(blocks_for_bits::<C::Block>(capacity)),
            nbits: 0,
        }
    }
    ...
    /// *length* of the returned bitvector, but only the *capacity*.
    #[inline]
    pub fn with_capacity_general_in(capacity: usize, alloc: C::Alloc) -> Self {
        BitVec {
            storage: C::with_capacity_in(blocks_for_bits::<C::Block>(capacity), alloc),
            nbits: 0,
        }
    }
    ...
    pub fn from_bytes_general(bytes: &[u8]) -> Self {
        ...
        let mut bit_vec = BitVec::<C::Block, C>::with_capacity_general(len);
        let complete_words = bytes.len() / C::BYTES;
        let extra_bytes = bytes.len() % C::BYTES;
    ...
    /// last word.
    #[inline]
    fn process<F>(&mut self, other: &BitVec<C::Block, C>, mut op: F) -> bool
    where
        F: FnMut(C::Block, C::Block) -> C::Block,
    {
        assert_eq!(self.len(), other.len());
        debug_assert_eq!(self.storage.len(), other.storage.len());
        let mut changed_bits = C::ZERO;
    ...
    #[inline]
    fn last_block_with_mask(&self) -> Option<(C::Block, C::Block)> {
        let extra_bits = self.len() % C::BITS;
        if extra_bits > 0 {
            let mask = (C::ONE << extra_bits) - C::ONE;
    ...
    /// Helper for procedures involving spare space in the last block.
    #[inline]
    fn last_block_mut_with_mask(&mut self) -> Option<(&mut C::Block, C::Block)> {
        let extra_bits = self.len() % C::BITS;
        if extra_bits > 0 {
            let mask = (C::ONE << extra_bits) - C::ONE;
            let storage_len = self.storage.len();
    ...
    pub fn get(&self, i: usize) -> Option<bool> {
        ...
        self.storage
            .slice()
            .get(w)
            .map(|&block| (block & (C::ONE << b)) != C::ZERO)
    }
    ...
    pub fn all(&self) -> bool {
        ...
        }) && (last_word == mask_for_bits::<C::Block>(self.nbits))
    }
    ...
    pub fn count_zeros(&self) -> u64 {
        ...
        let extra_zeros = (C::BITS - (self.len() % C::BITS)) % C::BITS;
        self.blocks()
    ...
    pub fn append(&mut self, other: &mut Self) {
        ...
        let b = self.len() % C::BITS;
        let o = other.len() % C::BITS;
        let will_overflow = (b + o > C::BITS) || (o == 0 && b != 0);
        ...
        if b == 0 {
            ...
            for block in other.storage.drain(..) {
                ...
                self.storage.push(block >> (C::BITS - b));
            }
    ...
    pub fn split_off(&mut self, at: usize) -> Self {
        ...
        let mut other = BitVec::<C::Block, C>::new_general();
        ...
        if b == 0 {
...

            {
                ...
                for &cur in iter {
                    other.storage.push((last >> b) | (cur << (C::BITS - b)));
                    last = cur;
                }
    ...
    pub fn truncate(&mut self, len: usize) {
        ...
        if len < self.len() {
            self.nbits = len;
            // This fixes (2).
            self.storage.truncate(blocks_for_bits::<C::Block>(len));
            self.fix_last_block();
        }
    }
    ...
    pub fn reserve(&mut self, additional: usize) {
        ...
        if desired_cap > self.capacity() {
            self.storage
                .reserve(blocks_for_bits::<C::Block>(desired_cap) - storage_len);
        }
    }
    ...
    pub fn reserve_exact(&mut self, additional: usize) {
        ...
        if desired_cap > self.capacity() {
            self.storage
                .reserve_exact(blocks_for_bits::<C::Block>(desired_cap) - storage_len);
        }
    }
    ...
    pub fn capacity(&self) -> usize {
        self.storage.capacity().saturating_mul(C::BITS)
    }
    ...
    pub fn grow(&mut self, n: usize, value: bool) {
        ...
        let new_nblocks = blocks_for_bits::<C::Block>(new_nbits);
        let full_value = if value { !C::ZERO } else { C::ZERO };

        // Correct the old tail word, setting or clearing formerly unused bits
        let num_cur_blocks = blocks_for_bits::<C::Block>(self.nbits);
        if self.nbits % C::BITS > 0 {
            let mask = mask_for_bits::<C::Block>(self.nbits);
            if value {
    ...
    pub fn fill(&mut self, bit: bool) {
        self.ensure_invariant();
        let block = if bit { !C::ZERO } else { C::ZERO };
        for w in self.storage.slice_mut() {
            *w = block;
        }
        if bit {
    ...
    pub fn insert(&mut self, at: usize, bit: bool) {
        ...
        let last_block_bits = self.nbits % C::BITS;
        let block_at = at / C::BITS; // needed block
        let bit_at = at % C::BITS; // index within the block
        ...
        let mut carry = self.storage.slice()[block_at] >> (C::BITS - 1);
        let lsbits_mask = (C::ONE << bit_at) - C::ONE;
        let set_bit = if bit { C::ONE } else { C::ZERO } << bit_at;
        ...
        for block_ref in &mut self.storage.slice_mut()[block_at + 1..] {
            let curr_carry = *block_ref >> (C::BITS - 1);
    ...
    pub fn push_within_capacity(&mut self, bit: bool) -> Result<(), bool> {
        ...
        let bit_at = len % bits;
        let flag = if bit { C::ONE << bit_at } else { C::ZERO };
...
impl<C: BitContainer> Default for BitVec<C::Block, C> {
    #[inline]
    fn default() -> Self {
        BitVec {
            storage: C::new_in(Default::default()),
            nbits: 0,
        }
    }
}

impl<C: BitContainer> FromIterator<bool> for BitVec<C::Block, C> {
    #[inline]
...
impl<C: BitContainer> Extend<bool> for BitVec<C::Block, C> {
    #[inline]
...
impl<C: BitContainer> Clone for BitVec<C::Block, C> {
    #[inline]
    fn clone(&self) -> Self {
        self.ensure_invariant();
        BitVec {
...
impl<C: BitContainer> PartialOrd for BitVec<C::Block, C> {
    #[inline]
...
impl<C: BitContainer> Ord for BitVec<C::Block, C> {
    #[inline]
...
impl<C: BitContainer> fmt::Display for BitVec<C::Block, C> {
...
impl<C: BitContainer> fmt::Debug for BitVec<C::Block, C> {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        self.ensure_invariant();
        let mut storage = String::with_capacity(self.len() + self.len() / C::BITS);
...
impl<C: BitContainer> hash::Hash for BitVec<C::Block, C> {
    #[inline]
...
impl<C: BitContainer> cmp::PartialEq for BitVec<C::Block, C> {
    #[inline]
...
impl<C: BitContainer> cmp::Eq for BitVec<C::Block, C> {}
