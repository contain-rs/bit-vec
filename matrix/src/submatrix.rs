...
pub struct BitSubMatrix<'a, B: BitBlockOrStore> {
    pub(crate) slice: &'a [Block<B>],
    pub(crate) row_bits: usize,
}

/// Mutable access to a range of matrix's rows.
pub struct BitSubMatrixMut<'a, B: BitBlockOrStore> {
    pub(crate) slice: &'a mut [Block<B>],
    pub(crate) row_bits: usize,
}

impl<'a, B: BitBlockOrStore> BitSubMatrix<'a, B> {
    // /// Returns a new BitSubMatrix.
    // pub(crate) fn new(slice: &[Block<B>], row_bits: usize) -> BitSubMatrix<'_, B> {
    ...
    /// unsafe.
    #[inline]
    pub unsafe fn from_raw_parts(ptr: *const Block<B>, rows: usize, row_bits: usize) -> Self {
        BitSubMatrix {
    ...
    pub fn iter(&self) -> impl Iterator<Item = &BitSlice<Block<B>>> {
        fn f<B: BitBlockOrStore>(arg: &[Block<B>]) -> &BitSlice<Block<B>> {
...
impl<'a, B: BitBlockOrStore> BitSubMatrixMut<'a, B> {
    /// Returns a new `BitSubMatrixMut`.
    pub(crate) fn new(slice: &mut [Block<B>], row_bits: usize) -> BitSubMatrixMut<'_, B> {
    ...
    #[inline]
    pub unsafe fn from_raw_parts(ptr: *mut Block<B>, rows: usize, row_bits: usize) -> Self {
        BitSubMatrixMut {
    ...
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut BitSlice<Block<B>>> {
        fn f<B: BitBlockOrStore>(arg: &mut [Block<B>]) -> &mut BitSlice<Block<B>> {
            unsafe { mem::transmute(arg) }
        }
...
impl<'a, B: BitBlockOrStore> Index<usize> for BitSubMatrixMut<'a, B> {
...
/// Returns the matrix's row in the form of a mutable slice.
impl<'a, B: BitBlockOrStore> IndexMut<usize> for BitSubMatrixMut<'a, B> {
    #[inline]
...
/// Returns the matrix's row in the form of a mutable slice.
impl<'a, B: BitBlockOrStore> Index<usize> for BitSubMatrix<'a, B> {
    type Output = BitSlice<Block<B>>;
...
impl<'a, B: BitBlockOrStore> fmt::Debug for BitSubMatrix<'a, B> {
